...
この閾値は課題（VI.3.3-4）で明示的に指定された値をそのまま採用しています。各分岐が呼び出す Simple / Medium / Complex は、上記の通りそれぞれ独立に O(n²) / O(n√n) / O(n log n) であることが保証されているため、disorder の値に応じてこの3つを振り分けるだけで、課題が要求する「disorder の区間ごとの計算量目標」を自動的に満たす設計になっています。
